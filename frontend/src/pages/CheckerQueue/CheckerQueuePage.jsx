 ...
 * Work queue for Checkers — shows active and historical review requests.
 */
...
const STATE_COLORS = {
    DESIGN_SUBMITTED:    { bg: '#ecfeff', color: '#0891b2', label: 'Design Submitted' },
...
export default function CheckerQueuePage() {
    ...
    const loadQueue = async (tab) => {
        setLoading(true);
        try {
            const r = await getWorkflowQueue('checker', { history: tab === 'history' });
        ...
        <div style={{ padding: '24px 20px', fontFamily: "'Inter','Segoe UI',sans-serif", width: '100%' }}>
            ...
            {loading ? (
                ...
                <div style={{
                    ...
                    {activeTab === 'history' 
                        ? 'No reviewed or completed designs yet.' 
